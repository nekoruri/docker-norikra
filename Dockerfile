...
RUN apt-get update && apt-get -y install build-essential curl git openjdk-7-jdk && curl -L http://jruby.org.s3.amazonaws.com/downloads/1.7.17/jruby-bin-1.7.17.tar.gz | tar zxf - && gem install norikra --no-ri --no-rdoc && apt-get -y remove build-essential && apt-get -y autoremove && apt-get -y clean
